...
// LOG payout lanes + HTTP TriggerVrf (round-watcher). One binary, config selects the path.
 ...
 * LOG: first iteration uses checkLog (validates the triggering log); later drains use checkUpkeep only.
 */
const runMigration = (runtime: Runtime<Config>, triggerLog?: EVMLog): string => {
  ...
  for (let i = 0; i < maxDrainIterations; i++) {
    ...
    if (config.migrationType === "LOG" && i === 0) {
...
function initWorkflow(config: Config) {
  ...
  if (config.migrationType === "HTTP") {
    ...
    if (authorizedKeys.length === 0 && !config.allowUnauthenticatedSim) {
      throw new Error(
        "authorizedKeys is required for HTTP migration (or set allowUnauthenticatedSim for local simulate only)",
      );
    }
