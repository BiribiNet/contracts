...
interface IBRBJackpotFunder {
    ...
    /// @notice Pulls `swapAssetTotalBps` of `marketWin` in `asset` from this contract balance, swaps to BRB when `asset != brb`, else splits BRB in-place; treasury receives its BRB share, remainder is burned (supply reduction).
