...
library RouletteLib {
    ...
    /**
     * @dev Worst-case single-spin liability (same closed-form decomposition as legacy `RouletteClean`),
     *      before the safety buffer. Pair/dozen/column terms use overlapping-outside optimality bounds.
     * @param otherBetsWeightedSum Σ(amount * payoutMultiplier) for split/corner/line/trio legs (same
     *      conservative rollup as legacy `roundOtherBetsPayout`).
     */
    function maxLiabilityRaw(
        uint256 maxStraightBet,
        uint256 maxStreetBet,
        uint256 redBetsSum,
        uint256 blackBetsSum,
        uint256 oddBetsSum,
        uint256 evenBetsSum,
        uint256 lowBetsSum,
        uint256 highBetsSum,
        ...
        uint256 column2,
        uint256 column3,
        uint256 otherBetsWeightedSum
    ) internal pure returns (uint256) {
        unchecked {
            uint256 straightStreet = (maxStraightBet * 36) + (maxStreetBet * 12);
            uint256 redBlackComponent = max(redBetsSum, blackBetsSum) * 2;
            uint256 oddEvenComponent = max(oddBetsSum, evenBetsSum) * 2;
            uint256 lowHighComponent = max(lowBetsSum, highBetsSum) * 2;
            uint256 dozenComponent = max3(dozen1, dozen2, dozen3) * 3;
            uint256 columnComponent = max3(column1, column2, column3) * 3;
            return straightStreet + redBlackComponent + oddEvenComponent + lowHighComponent + dozenComponent
                + columnComponent + otherBetsWeightedSum;
        }
    }

    /// @dev Apply `SAFETY_BUFFER_BPS` (110% default) to a raw liability from {maxLiabilityRaw}.
    function applySafetyBuffer(uint256 rawLiability) internal pure returns (uint256) {
        unchecked {
